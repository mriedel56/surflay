...
def change_image_width(workdir, prefix, plane, i):
    ...
    image=Image.open(op.join(workdir, '{prefix}_{plane}_{cut}.png'.format(prefix=prefix, plane=plane, cut=i)))
    image.load()
    ...
    new_image.save(op.join(workdir, '{prefix}_{plane}_{cut}.png'.format(prefix=prefix, plane=plane, cut=i)))
